...
- **Validated automatically.** `src/lib/questionEngine/validators.ts`
  ...
  `tests/questionEngine.test.ts` — **594 tests, all passing.**
...
question bank, playable end to end) — **26 of the 100 levels**:
...
| Year 5, Level 6 | 21 | Decimals to 3dp, percentages as parts-per-hundred, rounding decimals |
| Year 5, Level 7 | 21 | Area/perimeter of rectangles, volume/capacity, converting metric units |
...
| Year 2, Level 2 | 21 | Addition/subtraction facts to 20, deriving related facts to 100, mental number-line methods |
...
Every one of these 26 levels' templates is individually verified (by
...
**The other 74 levels** have their objectives fully defined but no lessons
...
per-template by the tests) but only for these 26 levels; it is not a claim
