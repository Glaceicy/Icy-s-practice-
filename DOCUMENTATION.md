...
- **Validated automatically.** `src/lib/questionEngine/validators.ts`
  ...
  `tests/questionEngine.test.ts` — **462 tests, all passing.**
...
question bank, playable end to end) — **20 of the 100 levels**:
...
Every one of these 20 levels' templates is individually verified (by
...
**The other 80 levels** have their objectives fully defined but no lessons
...
per-template by the tests) but only for these 20 levels; it is not a claim
