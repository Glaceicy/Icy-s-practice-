...
- **Validated automatically.** `src/lib/questionEngine/validators.ts`
  ...
  `tests/questionEngine.test.ts` — **616 tests, all passing.**
...
question bank, playable end to end) — **27 of the 100 levels**:
...
Every one of these 27 levels' templates is individually verified (by
...
**The other 73 levels** have their objectives fully defined but no lessons
...
per-template by the tests) but only for these 27 levels; it is not a claim
